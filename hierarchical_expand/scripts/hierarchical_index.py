...
def get_embedding(text: str) -> List[float]:
    ...
    ollama_url = os.environ.get('OLLAMA_API_BASE', os.environ.get('R3_EMBED_URL', 'http://localhost:11434')).rstrip('/')
    if '/v1' in ollama_url:
    ...
    if _OLLAMA_ONLINE is False:
        raise RuntimeError('Ollama is offline or nomic-embed-text is unreachable. Embedding generation failed.')
    ...
    for attempt in range(5):
        try:
            if _OLLAMA_ONLINE is None or _OLLAMA_ONLINE is False:
                try:
                    with urllib.request.urlopen(ollama_url, timeout=2.0) as resp:
                        if resp.status == 200:
                            _OLLAMA_ONLINE = True
                except Exception:
                    _OLLAMA_ONLINE = False
                    if attempt == 4:
                        raise RuntimeError('Ollama connection timed out or is unreachable.')
            ...
            # Add a small delay after successful generation to prevent overloading Ollama
            time.sleep(0.15)
            return emb
        except Exception as e:
            if attempt == 4:
                _OLLAMA_ONLINE = False
                raise RuntimeError(f'Ollama embedding API call failed after 5 attempts: {e}')
            # Sleep significantly longer on failures to let Ollama flush memory / queues
            cool_down = 10 * attempt + 5
            print(f'[Ollama] Attempt {attempt+1} failed: {e}. Cooling down for {cool_down}s...')
...
class HierarchicalIndex:
    def __init__(self, dataset_dir: str, cache_file: str = "hierarchical_index_cache.json"):
        ...
        if cache_file == "hierarchical_index_cache.json":
            parent_cache = os.path.join(os.path.dirname(self.dataset_dir), "hierarchical_index_cache.json")
            if os.path.exists(parent_cache):
                cache_file = parent_cache
            else:
                try:
                    skills_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
                    skill_cache = os.path.join(skills_root, "hierarchical_index", "hierarchical_index_cache.json")
                    if os.path.exists(skill_cache):
                        cache_file = skill_cache
                except Exception:
                    pass
        ...
        if os.path.exists(self.cache_file):
            try:
                with open(self.cache_file, "r", encoding="utf-8") as f:
                    ...
                    cached_dir = cached_data.get("dataset_dir")
                    if cached_dir and os.path.abspath(cached_dir) == os.path.abspath(self.dataset_dir):
